...
@section('title', 'Add to Lab')
@section('page-title', 'Add to Lab')
@section('page-description', 'Add a new project to your development lab')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-gray-800 rounded-lg border border-gray-700 p-6">
        <form action="{{ route('admin.upcoming-projects.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea name="description" rows="4" required 
                          class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent text-white placeholder-gray-400"
                          placeholder="Describe what you're building...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="tech_stack" value="{{ old('tech_stack') }}" required 
                       class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent text-white placeholder-gray-400"
                       placeholder="Laravel, Vue.js, MySQL, Redis (comma separated)">
            ...
            <div class="grid md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="expected_completion" value="{{ old('expected_completion') }}" required 
                           class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent text-white">
            ...
            <div>
                ...
                <input type="text" name="current_phase" value="{{ old('current_phase') }}" 
                       class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent text-white placeholder-gray-400"
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-700">
                <a href="{{ route('admin.upcoming-projects.index') }}" class="px-6 py-3 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition-colors">
                    Cancel
                </a>
                <button type="submit" class="px-6 py-3 bg-primary-600 hover:bg-primary-700 text-white rounded-lg transition-colors">
                    <i class="fas fa-flask mr-2"></i>Add to Lab
                </button>
            </div>
        </form>
    </div>
